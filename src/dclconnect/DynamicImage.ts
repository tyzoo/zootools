...
export class DynamicImage implements ISystem{
    ...
    moveTo(to: Vector2, duration: number, isPercent: boolean = false){
        if(this.animate){
            let { positionX, positionY } = this.image
            let from = new Vector2(Number(positionX), Number(positionY))
    ...
    resizeTo(to: Vector2, duration: number, isPercent: boolean = false){
        if(this.animate){
            ...
            let from = new Vector2(Number(width), Number(height))
    ...
    private handleSize(dt: number){
        if(this.animations.size){
            ...
            if(
                this.image.width == options.to.x &&
                this.image.height == options.to.y
            ){
    ...
    private handlePosition(dt: number){
        if(this.animations.position){
            ...
            if(
                this.image.positionX == options.to.x &&
                this.image.positionY == options.to.y
            ){
